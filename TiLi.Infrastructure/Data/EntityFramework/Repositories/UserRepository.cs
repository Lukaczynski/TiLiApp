  ...
  internal sealed class UserRepository : IUserRepository
  {
    ...
    public async Task<CreateUserResponse> Create(User user, string password)
    {
      var appUser = _mapper.Map<AppUser>(user);
            ...
            return new CreateUserResponse(appUser.Id, true, null );//  identityResult.Succeeded ? null : identityResult.Errors.Select(e => new Error(e.Code, e.Description)));
    }

      public async Task<User> FindByName(string userName)
      {
            return new User("Test", "TestLastname", "testEmail", userName);//_mapper.Map<User>(await _userManager.FindByNameAsync(userName));
      }
