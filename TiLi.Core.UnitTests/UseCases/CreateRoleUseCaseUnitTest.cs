...
namespace TiLi.Core.UnitTests.UseCases
{
    public class CreateRoleUseCaseUnitTest
    {
        ...
        public async void Can_Add_Rol()
        {
            ...
            var mockProjectRepository = new Mock<IRoleRepository>();
            mockProjectRepository
              .Setup(repo => repo.Create(It.IsAny<Role>()))
                  .Returns(Task.FromResult(new Dto.GatewayResponses.CreateBaseResponseDTO("", true)));
            // 2. The use case and star of this test
            var useCase = new CreateRoleUseCase(mockProjectRepository.Object);
            ...
            var mockOutputPort = new Mock<IOutputPort<CreateRoleResponse>>();
            mockOutputPort.Setup(outputPort => outputPort.Handle(It.IsAny<CreateRoleResponse>()));

            // act

            // 4. We need a request model to carry data into the use case from the upper layer (UI, Controller etc.)
            var response = await useCase.Handle(new CreateRoleRequest("RolTester"), mockOutputPort.Object);
